...
___mkd_freefootnotes(MMIOT *f)
{
    ...
    if ( f->footnotes ) {
	for (i=0; i < MKD_S(*f->footnotes); i++) {
	    MKD_DELETE(MKD_T(*f->footnotes)[i].tag);
	    MKD_DELETE(MKD_T(*f->footnotes)[i].link);
	    MKD_DELETE(MKD_T(*f->footnotes)[i].title);
	}
	MKD_DELETE(*f->footnotes);
	free(f->footnotes);
    }
}
...
mkd_cleanup(Document *doc)
{
	...
	if ( MKD_T(doc->content) ) ___mkd_freeLines(MKD_T(doc->content));
