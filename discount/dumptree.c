...
typedef MKD_STRING(struct frame) Stack;
...
changepfx(Stack *sp, char c)
{
    ...
    ch = MKD_T(*sp)[MKD_S(*sp)-1].c;

    if ( ch == '+' || ch == '|' )
	MKD_T(*sp)[MKD_S(*sp)-1].c = c;
}
...
printpfx(Stack *sp, FILE *f)
{
	...
	for ( i=0; i < MKD_S(*sp); i++ ) {
	    if ( i )
		fprintf(f, "  ");
	    fprintf(f, "%*s%c", MKD_T(*sp)[i].indent + 2, " ", MKD_T(*sp)[i].c);
	    if ( MKD_T(*sp)[i].c == '`' )
		MKD_T(*sp)[i].c = ' ';
	}
    fprintf(f, "--");
