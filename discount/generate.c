...
peek(MMIOT *f, int i)
{
    ...
    return (i >= 0) && (i < MKD_S(f->in)) ? MKD_T(f->in)[i] : EOF;
}
...
pull(MMIOT *f)
{
    return ( f->isp < MKD_S(f->in) ) ? MKD_T(f->in)[f->isp++] : EOF;
}
...
Qchar(char c, MMIOT *f)
{
	...
	cur = &MKD_T(f->Q)[MKD_S(f->Q)-1];

    MKD_EXPAND(cur->b_text) = c;
...
Qem(MMIOT *f, char c, int count)
{
    ...
    memset(&MKD_EXPAND(f->Q), 0, sizeof(block));
}
...
{
    MKD_PREFIX(*s, emtags[level-1].close, emtags[level-1].size);
}
...
{
    MKD_SUFFIX(*s, emtags[level-1].open, emtags[level-1].size-1);
}
...
emmatch(MMIOT *f, int go)
{
	...
	if ( e != EOF ) {
	    ...
	    for (i=0; i < match; i++)
		MKD_EXPAND(start->b_text) = start->b_char;
	}
...
emblock(MMIOT *f)
{
    ...
    for (i=0; i < MKD_S(f->Q); i++) {
	p = &MKD_T(f->Q)[i];
	
	if ( p->b_type != bTEXT ) emmatch(f, i);

	if ( MKD_S(p->b_post) ) { MKD_SUFFIX(f->out, MKD_T(p->b_post), MKD_S(p->b_post));
			      MKD_DELETE(p->b_post); }
	if ( MKD_S(p->b_text) ) { MKD_SUFFIX(f->out, MKD_T(p->b_text), MKD_S(p->b_text));
			      MKD_DELETE(p->b_text); }
    }
    MKD_S(f->Q) = 0;
}
...
reparse(char *bfr, int size, int flags, MMIOT *f)
{
    ...
    Qwrite(MKD_T(sub.out), MKD_S(sub.out), f);
...
linkykey(int image, Footnote *val, MMIOT *f)
{
    ...
    if ( (MKD_T(val->tag) = linkylabel(f, &MKD_S(val->tag))) == 0 )
	return 0;
    ...
    case '(':
	/* embedded link */
	if ( (MKD_T(val->link) = linkyurl(f,&MKD_S(val->link))) == 0 )
	    return 0;

	if ( image && !linkysize(f, &val->height, &val->width) )
	    return 0;

	MKD_T(val->title) = linkytitle(f, &MKD_S(val->title));
	...
	ret = bsearch(val, MKD_T(*f->footnotes), MKD_S(*f->footnotes),
...
static linkytype imaget = { 0, 0, "<img src=\"", "\"",
			     1, " alt=\"", "\" />", MKD_DENY_IMG|MKD_INSIDE_TAG };
...
extratag(Cstring t)
{
	...
	r = &specials[i];
	if ( (MKD_S(t) > r->szpat) && (strncasecmp(MKD_T(t), r->pat, r->szpat) == 0) )
	    return r;
    }
    return 0;
}
...
linkylinky(int image, MMIOT *f)
{
    ...
    if ( image )
	tag = &imaget;
    else if ( (f->flags & MKD_NO_PSEUDO_PROTO) || (tag = extratag(link.link)) == 0 )
	tag = &linkt;
	...
	if ( f->base && (MKD_T(link.link)[tag->szpat] == '/') )
	    puturl(f->base, strlen(f->base), f);
	puturl(MKD_T(link.link) + tag->szpat, MKD_S(link.link) - tag->szpat, f);
	...
	if ( MKD_S(link.title) ) {
	    Qstring(" title=\"", f);
	    reparse(MKD_T(link.title), MKD_S(link.title), MKD_INSIDE_TAG, f);
	    Qchar('"', f);
	}

	Qstring(tag->text_pfx, f);
	reparse(MKD_T(link.tag), MKD_S(link.tag), tag->flags, f);
	Qstring(tag->text_sfx, f);
    }
    else
	Qwrite(MKD_T(link.link) + tag->szpat, MKD_S(link.link) - tag->szpat, f);
...
forbidden_tag(MMIOT *f)
{
    ...
    if ( f->flags & MKD_DENY_HTML )
	return 1;

    if ( c == 'A' && (f->flags & MKD_DENY_A) && !isthisalnum(f,2) )
	return 1;
    if ( c == 'I' && (f->flags & MKD_DENY_IMG)
...
maybe_tag_or_link(MMIOT *f)
{
    ...
    if ( f->flags & MKD_DENY_A ) return 0;
...
#define tag_text(f)	(f->flags & MKD_INSIDE_TAG)
...
#if SUPERSCRIPT
	/* A^B -> A<sup>B</sup> */
	case '^':   if ( (f->flags & (MKD_STRICT|MKD_INSIDE_TAG)) || isthisspace(f,-1) || isthisspace(f,1) )
			Qchar(c,f);
		    else {
			char *sup = cursor(f);
			int len = 0;
...
printheader(Paragraph *pp, MMIOT *f)
{
	...
	mkd_string_to_anchor(MKD_T(pp->text->text), MKD_S(pp->text->text), Qchar, f);
	Qchar('"', f);
    }
    Qchar('>', f);
    push(MKD_T(pp->text->text), MKD_S(pp->text->text), f);
    text(f);
    Qprintf(f, "</h%d>", pp->hnumber);
}
...
printblock(Paragraph *pp, MMIOT *f)
{
    ...
    while (t) {
	if ( MKD_S(t->text) ) {
	    if ( MKD_S(t->text) > 2 && MKD_T(t->text)[MKD_S(t->text)-2] == ' '
				&& MKD_T(t->text)[MKD_S(t->text)-1] == ' ') {
		push(MKD_T(t->text), MKD_S(t->text)-2, f);
		push("<br/>\n", 6, f);
	    }
	    else {
		push(MKD_T(t->text), MKD_S(t->text), f);
		if ( t->next )
		    push("\n", 1, f);
	    }
	}
...
printhtml(Line *t, MMIOT *f)
{
	...
	if ( MKD_S(t->text) ) {
	    ...
	    Qwrite(MKD_T(t->text), MKD_S(t->text), f);
	    Qchar('\n', f);
	}
	else
	    blanks++;
...
definitionlist(Paragraph *p, MMIOT *f)
{
	...
	for ( ; p ; p = p->next) {
		...
		Qstring("<dt>", f);
		reparse(MKD_T(tag->text), MKD_S(tag->text), 0, f);
		Qstring("</dt>\n", f);
	    }
...
stylesheets(Paragraph *p, FILE *f)
{
	...
	if ( p->typ == STYLE ) {
		...
		if ( fwrite(MKD_T(q->text), MKD_S(q->text), 1, f) == 1 )
		    putc('\n', f);
		else
		    return EOF;
	}
...
mkd_text(char *bfr, int size, FILE *output, int flags)
{
    ...
    f.flags = flags & MKD_USER_FLAGS;
    
    reparse(bfr, size, 0, &f);
    emblock(&f);
    if ( flags & MKD_CDATA_OUTPUT )
	___mkd_xml(MKD_T(f.out), MKD_S(f.out), output);
    else
	fwrite(MKD_T(f.out), MKD_S(f.out), 1, output);
