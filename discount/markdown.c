...
typedef MKD_ANCHOR(Paragraph) ParagraphRoot;
...
__mkd_footsort(Footnote *a, Footnote *b)
{
    ...
    if ( MKD_S(a->tag) != MKD_S(b->tag) )
	return MKD_S(a->tag) - MKD_S(b->tag);

    for ( i=0; i < MKD_S(a->tag); i++) {
	ac = tolower(MKD_T(a->tag)[i]);
	bc = tolower(MKD_T(b->tag)[i]);
...
{
    while ( (i < MKD_S(t->text)) && !isspace(MKD_T(t->text)[i]) )
	++i;
    return i;
}
...
isopentag(Line *p)
{
    ...
    if ( len < 3 || MKD_T(p->text)[0] != '<' )
	return 0;

    /* find how long the tag is so we can check to see if
     * it's a block-level tag
     */
    for ( i=1; i < len && MKD_T(p->text)[i] != '>' 
		       && MKD_T(p->text)[i] != '/'
		       && !isspace(MKD_T(p->text)[i]); ++i )
	;

    key.id = MKD_T(p->text)+1;
    key.siz = i-1;
...
htmlblock(Paragraph *p, char *tag)
{
    ...
    for ( ; t ; t = t->next) {
	if ( strncasecmp(MKD_T(t->text), close, closesize) == 0 ) {
	    ret = t->next;
	    t->next = 0;
	    return ret;
	}
...
isfootnote(Line *t)
{
    ...
    if ( ( (i = t->dle) > 3) || (MKD_T(t->text)[i] != '[') )
	return 0;

    for ( ++i; i < MKD_S(t->text) ; ++i ) {
	if ( MKD_T(t->text)[i] == '[' )
	    return 0;
	else if ( MKD_T(t->text)[i] == ']' && MKD_T(t->text)[i+1] == ':' )
	    return 1;
    }
    return 0;
}
...
ishdr(Line *t, int *htyp)
{
	...
	*htyp = ETX;
    ...
    if ( t->next ) {
	char *q = MKD_T(t->next->text);

	if ( (*q == '=') || (*q == '-') ) {
	    for (i=1; i < MKD_S(t->next->text); i++)
		if ( q[0] != q[i] )
		    return 0;
	    *htyp = SETEXT;
	    return 1;
...
islist(Line *t, int *trim)
{
	...
	*trim = 4;
    ...
    if ( strchr("*-+", MKD_T(t->text)[t->dle]) && isspace(MKD_T(t->text)[t->dle+1]) ) {
	...
	*trim = (i > 4) ? 4 : i;
	...
	if ( MKD_T(t->text)[j-1] == '.' ) {
#if ALPHA_LIST
	if ( (j == t->dle + 2) && isalpha(MKD_T(t->text)[t->dle]) ) {
	    j = nextnonblank(t,j);
	    *trim = j;
	    return AL;
	}
#endif
	    strtoul(MKD_T(t->text)+t->dle, &q, 10);
	    if ( (q > MKD_T(t->text)+t->dle) && (q == MKD_T(t->text) + (j-1)) ) {
		j = nextnonblank(t,j);
		*trim = j;
		return OL;
	    }
...
headerblock(Paragraph *pp, int htyp)
{
    ...
    case SETEXT:
	     ...
	     */
	    pp->hnumber = (MKD_T(p->next->text)[0] == '=') ? 1 : 2;
    ...
    case ETX:
	    ...
	    for (i=0; (MKD_T(p->text)[i] == MKD_T(p->text)[0]) && (i < MKD_S(p->text)-1); i++)
		;

	    pp->hnumber = i;

	    while ( (i < MKD_S(p->text)) && isspace(MKD_T(p->text)[i]) )
		++i;

	    MKD_CLIP(p->text, 0, i);

	    for (j=MKD_S(p->text); (j > 1) && (MKD_T(p->text)[j-1] == '#'); --j)
		;

	    while ( j && isspace(MKD_T(p->text)[j-1]) )
		--j;
...
codeblock(Paragraph *p)
{
     ...
     */
    while ( MKD_S(t->text) && isspace(MKD_T(t->text)[MKD_S(t->text)-1]) )
	--MKD_S(t->text);
...
centered(Line *first, Line *last)
{
    ...
    if ( first&&last ) {
	int len = MKD_S(last->text);

	if ( (len > 2) && (strncmp(MKD_T(first->text), "->", 2) == 0)
		       && (strncmp(MKD_T(last->text)+len-2, "<-", 2) == 0) ) {
	    MKD_CLIP(first->text, 0, 2);
	    MKD_S(last->text) -= 2;
	    return CENTER;
	}
    }
...
quoteblock(Paragraph *p)
{
    ...
    for ( t = p->text; t ; t = q ) {
	if ( isquote(t) ) {
	    qp = (MKD_T(t->text)[1] == ' ') ? 2 : 1;
	    MKD_CLIP(t->text, 0, qp);
	    t->dle = mkd_firstnonblank(t);
	}
	...
	if ( (i = szmarkerclass(1+MKD_T(q->text))) == 3 )
	    /* and this would be an "%id:" prefix */
	    prefix="id";
	    
	if ( (p->ident = malloc(4+i+MKD_S(q->text))) )
	    sprintf(p->ident, "%s=\"%.*s\"", prefix, MKD_S(q->text)-(i+2),
						     MKD_T(q->text)+(i+1) );
...
addfootnote(Line *p, MMIOT* f)
{
    ...
    Footnote *foot = &MKD_EXPAND(*f->footnotes);
    
    MKD_CREATE(foot->tag);
    MKD_CREATE(foot->link);
    MKD_CREATE(foot->title);
    foot->height = foot->width = 0;

    for (j=i=p->dle+1; MKD_T(p->text)[j] != ']'; j++)
	MKD_EXPAND(foot->tag) = MKD_T(p->text)[j];

    MKD_EXPAND(foot->tag) = 0;
    MKD_S(foot->tag)--;
    j = nextnonblank(p, j+2);

    while ( (j < MKD_S(p->text)) && !isspace(MKD_T(p->text)[j]) )
	MKD_EXPAND(foot->link) = MKD_T(p->text)[j++];
    MKD_EXPAND(foot->link) = 0;
    MKD_S(foot->link)--;
    j = nextnonblank(p,j);

    if ( MKD_T(p->text)[j] == '=' ) {
	sscanf(MKD_T(p->text)+j, "=%dx%d", &foot->width, &foot->height);
	while ( (j < MKD_S(p->text)) && !isspace(MKD_T(p->text)[j]) )
	    ++j;
	j = nextnonblank(p,j);
    }
    ...
    if ( (j >= MKD_S(p->text)) && np && np->dle && tgood(MKD_T(np->text)[np->dle]) ) {
	___mkd_freeLine(p);
	p = np;
	np = p->next;
	j = p->dle;
	...
	while ( j < MKD_S(p->text) )
	    MKD_EXPAND(foot->title) = MKD_T(p->text)[j++];

	while ( MKD_S(foot->title) && MKD_T(foot->title)[MKD_S(foot->title)-1] != c )
	    --MKD_S(foot->title);
	if ( MKD_S(foot->title) )	/* skip trailing quote */
	    --MKD_S(foot->title);
	MKD_EXPAND(foot->title) = 0;
	--MKD_S(foot->title);
    }
...
compile(Line *ptr, int toplevel, MMIOT *f)
{
    ...
    while ( ptr ) {
	if ( toplevel && !(f->flags & MKD_DENY_HTML) && (key = isopentag(ptr)) ) {
...
mkd_compile(Document *doc, int flags)
{
    ...
    doc->ctx->flags = flags & MKD_USER_FLAGS;
    doc->ctx->base = doc->base;
    MKD_CREATE(doc->ctx->in);
    doc->ctx->footnotes = malloc(sizeof doc->ctx->footnotes[0]);
    MKD_CREATE(*doc->ctx->footnotes);

    initialize();

    doc->code = compile(MKD_T(doc->content), 1, doc->ctx);
    qsort(MKD_T(*doc->ctx->footnotes), MKD_S(*doc->ctx->footnotes),
		        sizeof MKD_T(*doc->ctx->footnotes)[0],
			           (stfu)__mkd_footsort);
