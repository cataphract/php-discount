...
quoteblock(Paragraph *p, DWORD flags)
{
	...
	if ( p->ident = emalloc(4+strlen(prefix)+S(q->text)) )
...
addfootnote(Line *p, MMIOT* f)
{
    ...
    CREATE(foot->link);
    CREATE(foot->title);
    foot->height = foot->width = foot->flags = 0;
...
{
    Paragraph *ret = ecalloc(sizeof *ret, 1);
...
mkd_compile(Document *doc, DWORD flags)
{
    ...
    CREATE(doc->ctx->in);
    doc->ctx->footnotes = emalloc(sizeof doc->ctx->footnotes[0]);
...
/* on merge: added by me */
int
mkd_is_compiled(Document *doc) {
	return doc && doc->compiled;
}
